...
def deal_small_title_url(url_set):
    """Process URLs to extract specific identifiers from them."""
    cleaned_urls = []
    for url in url_set:
        if url.endswith('list.htm'):
            cleaned_urls.append(''.join(url.split('/')[:-1]))  # Remove 'list.htm'
        else:
...
def process_organization(big_institution_name, organization_name, organization_url):
    #print(f'{big_institution_name}-----{organization_name}-----{organization_url}')
    ...
    #print(f'{set_small_title_name}---{set_small_title_url}---{set_small_title_url}')
...
if __name__ == '__main__':
    ...
    # print(government_organization_name)
    # print(government_organization_url)
    # print(educational_organization_name)
    # print(educational_organization_url)
    with ProcessPoolExecutor() as executor:
        futures = []
        #Download government organizations
        for organization_name, organization_url in zip(government_organization_name, government_organization_url):
            #print(organization_name,organization_url)
            futures.append(
                executor.submit(process_organization, big_institution_name[0], organization_name, organization_url))
            #print(f'{process_organization}---{big_institution_name[0]}---{organization_name}---{organization_url}')

        # Download educational organizations
        for organization_name, organization_url in zip(educational_organization_name, educational_organization_url):
            #print(organization_name,organization_url)
            futures.append(
        ...
        for organization_name, organization_url in zip(scientific_institution_name, scientific_institution_url):
            #print(organization_name,organization_url)
            futures.append(
                executor.submit(process_organization, big_institution_name[2], organization_name, organization_url))
